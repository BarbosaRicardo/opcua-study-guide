    ...
    >
      ...
      >
        ...
        <div className="flex-1">
          <div className="text-xs font-bold text-purple-600 uppercase tracking-widest mb-0.5">Field Note</div>
          <div className="font-semibold text-slate-800 text-sm leading-relaxed">{analogy.text}</div>
          <div className="text-xs text-slate-500 mt-1">— {analogy.author}</div>
        </div>
        {open ? <ChevronUp size={18} className="text-purple-400 flex-shrink-0" /> : <ChevronDown size={18} className="text-purple-400 flex-shrink-0" />}
      </button>
    </motion.div>
  )
}
