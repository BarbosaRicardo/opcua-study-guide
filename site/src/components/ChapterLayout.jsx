  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-center mt-12 pt-6 border-t border-slate-100">
        {prevChapter ? (
          <Link to={`/${prevChapter.id === 'intro' ? '' : prevChapter.id}`} className="flex items-center gap-2 text-sm text-slate-500 hover:text-mblue-600 transition-colors">
            <ChevronLeft size={18} />
            <div>
        ...
        {nextChapter ? (
          <Link to={`/${nextChapter.id}`} className="flex items-center gap-2 text-sm text-slate-500 hover:text-mblue-600 transition-colors text-right">
            <div>
