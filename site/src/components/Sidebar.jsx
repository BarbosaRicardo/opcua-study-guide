...
import { Menu, X, Zap, BookOpen, BarChart2, Home, Layers, FolderTree, Bell, Shield, Radio, Network, Settings, Wrench, FlaskConical, CreditCard, LayoutGrid, LogIn, LogOut , FileText } from 'lucide-react'
...
const ICON_MAP = {
  BookOpen, Layers, FolderTree, Bell, Shield, Radio, Network, Settings, Wrench, FlaskConical, CreditCard,
}
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="p-5" style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0"
            style={{ background: 'linear-gradient(135deg, #0f766e, #0d9488)', boxShadow: '0 0 20px rgba(13,148,136,0.45)' }}>
            <Zap size={18} className="text-white" />
          </div>
          <div>
            <div className="font-black text-white text-sm tracking-wide leading-tight">OPC UA</div>
            <div className="text-xs font-medium" style={{ color: '#0d9488' }}>Study Guide</div>
          </div>
        </div>
        ...
        <div className="mt-4">
          <div className="flex justify-between text-xs mb-1.5">
            ...
            <span className="font-bold" style={{ color: '#0d9488' }}>{prog.pct}%</span>
          </div>
      ...
      <div className="p-4 space-y-3" style={{ borderTop: '1px solid rgba(13,148,136,0.12)' }}>
        <a
          ...
          style={{ background: 'linear-gradient(135deg,#0f766e,#0d9488)', boxShadow: '0 0 14px rgba(13,148,136,0.35)' }}
        >
        ...
        {!sessionLoading && (session ? (
          <div className="flex items-center justify-between gap-2 px-1">
            <span className="text-xs truncate" style={{ color: 'rgba(13,148,136,0.5)' }}>{session.user.email}</span>
            <button onClick={() => supabase.auth.signOut()} title="Sign out" style={{ color: 'rgba(13,148,136,0.5)' }} className="hover:text-rose-400 transition flex-shrink-0">
              <LogOut size={13} />
            </button>
          </div>
        ) : (
        ...
        <p className="text-center text-xs" style={{ color: 'rgba(13,148,136,0.35)' }}>
          OPC UA · IEC 62541 · May 2026
        </p>
      </div>
    </div>
  )
